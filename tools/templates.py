...
#        text = text.replace(m.group(), m.group(1) + ' ' + minutes, 1)
    ...
    for m in finditer(r'(\d+)-(часов[агеиймоухыюя]{2,3}|(силь|тон|каналь|странич|тысяч|миллион|миллиард|процент|секунд|минут|месяч|недель)н[агеиймоухыюя]{2,3}|лет[геиймнохюя]{2,4}|(|кило)граммов|(|кило|милли|санти)метров[агеиймоухыюя]{2,3})\b', text):
        ...
        elif len(m.group(1)) > 3 and m.group(1)[-2:] == '00':
            num = m.group(1)[:-3] + '000_' + cardinal(m.group(1)[-3:], r_ca)
        elif len(m.group(1)) > 2 and m.group(1)[-1:] == '0':
            num = m.group(1)[:-2] + '00_' + cardinal(m.group(1)[-2:], r_ca)
        else:
