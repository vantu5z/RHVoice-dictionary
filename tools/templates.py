...
#        text = text.replace(m.group(), m.group(1) + ' ' + minutes, 1)
    ...
    for m in finditer(r'(\d+)-(часов[агеиймоухыюя]{2,3}|(секунд|минут|месяч)н[агеиймоухыюя]{2,4}|лет[геиймнохюя]{2,4}|((|кило)граммов|(|кило|милли|санти)метров|тысяч|миллионн[агеиймоухыюя]{2,3}|миллиардн|процентн)[агеиймоухыюя]{2,3})\b', text):
        if m.group(1) == '100':
            num = 'сто'
        elif m.group(1) == '1000':
            num = 'тысяче'
    ...
    for m in finditer(r'\b([Вв] |[Нн]а |[Оо]б? |[Пп]ри )(\d+) ([а-я]+([ая]х|е|и))\b', text):
        ...
        if 20 > int(m.group(2)[-2:]) > 9:
            ...
            if m.group(2)[-1] == '1':
                if m.group(4) == 'е' or m.group(3) == 'и' or m.group(3) == 'у':
                    number = cardinal(m.group(2), p_ca)
                    if zh_dp.find('|' + m.group(3) + '|') != -1:
                        number = number[:-1] + 'й'
            else:
