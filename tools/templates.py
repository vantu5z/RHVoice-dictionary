...
forms = {
    ...
    'ГВт': ('мегаватт', 'мегаватта', 'мегаватт'),
    'Вт': ('ватт', 'ватта', 'ватт'),
    '%': ('процент', 'процента', 'процентов'),
    '₽': ('рубль', 'рубля', 'рублей'),
    '£': ('фунт стерлингов', 'фунта стерлингов', 'фунтов стерлингов'),
    'л\u002Eс\u002E': ('лошадиная сила', 'лошадиных силы', 'лошадиных сил')
}
...
samples = findall(r'(тыс\.|млн|млрд|трлн|тысяч[аи]?|миллион[аов]{,2}|миллиард[аов]{,2}|триллион[аов]{,2})(\$|£|₽)', text)
...
units = r'(%|°|℃|£|₽|\$|кг|км|мг|мм|м\b|см\b|т\b|кВт|МВт|ГВт|Вт|л\u002Eс\u002E)'
text = sub(r'(?<=\d) ' + units,  r'\1', text)
samples = findall(r'(\d+,\d+)' + units, text)
for sample in samples:
    text = text.replace(sample[0] + sample[1], sample[0] + ' ' + forms[sample[1]][1], 1)
...
text = sub(r'(ая|ых) ?тыс\.', r'\1 тысячи', text)
text = sub(r'(ая|ых) ?млн', r'\1 миллиона', text)
text = sub(r'(ая|ых) ?млрд', r'\1 миллиарда', text)
text = sub(r'(ая|ых) ?трлн', r'\1 триллиона', text)

text = sub(r'(?<=\d) ?тыс\.', '000', text)
text = sub(r'(?<=\d) ?млн', '000000', text)
text = sub(r'(?<=\d) ?млрд', '000000000', text)
text = sub(r'(?<=\d) ?трлн', '000000000000', text)
...
text = sub(r'(\d+)-(\d+-го)', r'\1-го \2', text)
