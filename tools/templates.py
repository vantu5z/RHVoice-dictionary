...
#        text = text.replace(m.group(), m.group(1) + ' ' + minutes, 1)
    ...
    for m in finditer(r'(\d+)-(часов[агеиймоухыюя]{2,3}|(тон|секунд|минут|месяч|каналь|тысяч|миллион|миллиард|процент|недель)н[агеиймоухыюя]{2,3}|лет[геиймнохюя]{2,4}|(|кило)граммов|(|кило|милли|санти)метров[агеиймоухыюя]{2,3})\b', text):
        if m.group(1) == '100':
            num = 'сто'
        elif m.group(1) == '1000':
            num = 'тысяче'
    ...
    # Дательный падеж
    for m in finditer(r'\b([Кк]о? |[Пп]о |равно )(\d+) ([а-я]+([ая]м|у|ю|е|и))\b', text):
        number = ''
        if m.group(3)[-1] == 'м':
            if len(m.group(2)) > 1:
    ...
    # Предложный падеж
    for m in finditer(r'\b([Вв]о? |[Нн]а |[Оо]б? |[Пп]ри )(\d+) ([а-я]+([ая]х|е|и|у))\b', text):
        number = ''
        if m.group(3)[-1] == 'х':
            if len(m.group(2)) > 1:
                if m.group(2)[-2] == '1' or m.group(2)[-1] != '1':
