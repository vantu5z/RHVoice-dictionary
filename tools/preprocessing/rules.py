...
class UnitRule_4(RuleBase):
    ...
    def __init__(self):
        super().__init__(
            ...
            r'[Бб]лиже|[Дд]альше|[Мм]оложе|[Сс]тарше|[Сс]выше|[Оо]коло|[Дд]ля|'
            r'[Дд]о|[Ии]з|[Оо]т|[Вв]место|[Дд]линнее|[Вв] размере|[Вв] течение|'
            r'[Вв] количестве|[Вв] пределах|[Дд]остигающ[аеиймухя]{1,3}|'
...
class CardinalRule_13(RuleBase):
    ...
    def __init__(self):
        super().__init__(
            ...
            r'[Зз]а исключением|[Кк]роме|[Пп]омимо|[Дд]остигающ[аеиймухя]{1,3}|'
            ...
            r')'
            r'( приблизительно | примерно | почти | более чем | менее чем '
            r'| плюс | минус | максимум | минимум | е[её] | его | их | всех '
            r'| [а-яё]+[иы]х | )'
            r'((\d+[,.]|)(\d+)( - | или )|)(\d+[,.]|)(\d+)'
...
class CardinalRule_15(RuleBase):
    """
    Описание: Количественные числительные. Родительный падеж.
    Пример: "21 дня -> двадцати одного дня"
    """
    def __init__(self):
        super().__init__(
            r'(\A|\n|\(| )((\d+)( - | или )|)(1|\d*[02-9]1)'
...
class CardinalRule_18(RuleBase):
    ...
    Пример: "2 миллионами -> двумя миллионами"
    """
    def __init__(self):
        super().__init__(
            r'(?<![,.;:])\b('
...
class CardinalRule_25(RuleBase):
    ...
    def check(self, m):
        ...
        self.add_debug_info(word=m.group(13), attrs=attr)
        ...
        if m.group(8):
            if (attr.have([M_GENDER, S_GENDER, Z_GENDER], None, [1])
                ...
                number = decimal(m.group(8)[:-1], m.group(9), 5)
            else:
                return None
        else:
            number = cardinal(m.group(9), v_ca)
            if attr.have([M_GENDER], False, [0, 3], all_case=True):
                pass
            elif number[-3:] == 'дин':
                if m.group(13) in ('сутки', 'брюки', 'ножницы'):
                    number = number[:-2] + 'ни'
                elif c:
                ...
                if m.group(13) in ('суток', 'брюк', 'ножниц'):
                    number = number[:-1] + 'ое'
                elif m.group(13) in ('сутки', 'брюки', 'ножницы'):
                    return None
        ...
        if m.group(4):
            if m.group(5):
                pre = decimal(m.group(5)[:-1], m.group(6), 5)
            else:
                pre = cardinal(m.group(6), v_ca)
                if pre[-3:] == 'дин':
                    if m.group(13) in ('сутки', 'суток', 'брюки', 'брюк',
                                       'ножницы', 'ножниц'):
                        pre = pre[:-2] + 'ни'
            ...
            pre = ''
        return m.group(1) + pre + number + m.group(10)
...
class CardinalRule_31(RuleBase):
    ...
    def __init__(self):
        super().__init__(
            r'(?<!-)\b([Кк] |[Бб]лагодаря |[Вв]опреки |'
            r'рав[авгеийлмностухшщьыюя]{2,9} |'
...
class CardinalRule_37(RuleBase):
    ...
    def __init__(self):
        super().__init__(
            ...
            r'светового года|световых годах?|'
            r'(тысячи|миллиона|миллиарда) (километров|световых лет))\b')
...
rules_list = (UnitRule_2(),         # следует перед UnitRule_10 и UnitRule_13
              ...
              UnitRule_17(),        # родительный (следует после UnitRule_5)
